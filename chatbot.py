...
class FinancialChatbot:
    ...
    def process_data(self, df):
        ...
        is_nubank = all(col in df.columns for col in [
                        'date', 'title', 'amount'])
    ...
    def analyze_data(self, df):
        ...
        insights['saldo'] = insights['receitas']['total'] - \
            insights['despesas']['total']
        ...
        if not despesas.empty and 'Categoria' in despesas.columns:
            top_categories = despesas.groupby(
                'Categoria')['Valor_Absoluto'].sum().sort_values(ascending=False)
...
""")
    ...
    def format_context_for_llm(self, context: Dict) -> str:
        ...
        context_parts.append(
            f"PERÍODO: {context.get('periodo', {}).get('inicio', 'N/A')} até {context.get('periodo', {}).get('fim', 'N/A')}")
        context_parts.append(
            f"TOTAL DE TRANSAÇÕES: {context.get('total_transacoes', 0):,}")

        # Receitas e despesas
        receitas = context.get('receitas', {})
        despesas = context.get('despesas', {})
        context_parts.append(
            f"RECEITAS: R$ {receitas.get('total', 0):,.2f} ({receitas.get('quantidade', 0)} transações)")
        context_parts.append(
            f"DESPESAS: R$ {despesas.get('total', 0):,.2f} ({despesas.get('quantidade', 0)} transações)")
        ...
        if gastos_mensais:
            context_parts.append(
                f"GASTO MENSAL MÉDIO: R$ {gastos_mensais.get('media', 0):,.2f}")
            context_parts.append(
                f"MAIOR GASTO MENSAL: R$ {gastos_mensais.get('maior', 0):,.2f} em {gastos_mensais.get('mes_maior_gasto', 'N/A')}")
        ...
        if padroes:
            context_parts.append(
                f"DIA DA SEMANA COM MAIS GASTOS: {padroes.get('dia_semana_mais_gasto', 'N/A')}")
            context_parts.append(
                f"MÊS COM MAIS GASTOS: {padroes.get('mes_mais_gasto', 'N/A')}")
...
def render_chatbot():
    ...
    if chatbot.df is None:
        ...
        if chatbot.df is not None:
            ...
            st.session_state.financial_context = chatbot.analyze_data(
                chatbot.df)
        else:
            st.warning(
                "⚠️ Nenhum dado financeiro encontrado. Coloque seus extratos CSV na pasta atual ou data/raw/")
    ...
    with st.sidebar:
        ...
        if context:
            ...
            st.metric("🔢 Transações",
                      f"{context.get('total_transacoes', 0):,}")
