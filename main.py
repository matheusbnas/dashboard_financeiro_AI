...
# Exemplo de uso para chaves de API
...
class FinancialDashboardLauncherV5:
    ...
    def save_config(self):
        """Salva configuração"""
        try:
            self.config["project_info"]["last_updated"] = datetime.now(
            ).isoformat()
    ...
    def check_data_availability(self) -> dict:
        ...
        if csv_files:
            ...
            stats["recent_files"] = [str(f[0])
                                     for f in csv_files_with_time[:5]]
            ...
            try:
                ...
                if all(col in sample_df.columns for col in ['date', 'title', 'amount']):
                    ...
                    # Formato tradicional
                    sample_df['Data'] = pd.to_datetime(
                        sample_df['Data'], errors='coerce')
                    stats["date_range"] = {
    ...
    def check_system_status(self) -> dict:
        ...
        for module_name, module_path in self.modules.items():
            status["modules_available"][module_name] = os.path.exists(
                module_path)
        ...
        # Verificar chatbot
        status["chatbot_ready"] = status["modules_available"].get(
            "chatbot", False)
        ...
        advanced_module = status["modules_available"].get(
            "sync_advanced", False)
        ...
        if env_file.exists():
            ...
            has_groq = "GROQ_API_KEY=" in env_content and len(
                env_content.split("GROQ_API_KEY=")[1].split("\n")[0].strip()) > 0
            has_openai = "OPENAI_API_KEY=" in env_content and len(
                env_content.split("OPENAI_API_KEY=")[1].split("\n")[0].strip()) > 0
    ...
    def show_welcome_screen_v5(self):
        ...
        print(
            "\033[93m" + "         DASHBOARD FINANCEIRO PESSOAL COMPLETO v5.0".center(100) + "\033[0m")
        ...
        print(f"\n📊 STATUS DO SISTEMA:")
        print(
            f"   • Arquivos CSV: {self.data_stats.get('csv_files', 0)} (Nubank: {self.data_stats.get('nubank_files', 0)})")
        print(
            f"   • Transações estimadas: {self.data_stats.get('total_transactions', 0):,}")
        ...
        features = [
            ...
            ("☁️ Google Sheets Básico",
             self.system_status["google_sheets_basic"]),
            ("🚀 Google Sheets Avançado",
             self.system_status["google_sheets_advanced"]),
            ("🏷️ Categorização IA", self.system_status["modules_available"].get(
                'categorizer', False)),
            ("📈 Análise Avançada", self.system_status["modules_available"].get(
                'analytics', False)),
        ...
        print(f"\n🔧 INTEGRAÇÕES:")
        print(
            f"   • Google Sheets Básico: {'✅ Configurado' if self.system_status['google_sheets_basic'] else '❌ Não configurado'}")
        print(
            f"   • Google Sheets Avançado: {'✅ Configurado' if self.system_status['google_sheets_advanced'] else '❌ Não configurado'}")
        print(
            f"   • IA (Groq/OpenAI): {'✅ Configurada' if self.system_status['ai_ready'] else '❌ Não configurada'}")

        # Dados Nubank
        if self.data_stats.get('nubank_files', 0) > 0:
            print(f"\n💳 DADOS NUBANK DETECTADOS:")
            print(
                f"   • {self.data_stats['nubank_files']} arquivos Nubank_*.csv")
        ...
        if self.config.get("first_run", True):
            ...
            print(
                f"   Recomendamos executar a configuração automática para otimizar o sistema.")
    ...
    def show_main_menu_v5(self):
        ...
        while True:
            ...
            selected_option = next(
                (opt for opt in options if opt[0] == choice), None)
            if selected_option:
    ...
    def execute_sheets_basic(self):
        ...
        print(
            f"   • Compartilhamento automático com: {self.emails['primary']}")
    ...
    def execute_data_manager(self):
        ...
        if choice == "1":
            ...
            print(
                f"   • Transações estimadas: {self.data_stats.get('total_transactions', 0):,}")
            print(
                f"   • Categorias disponíveis: {'Sim' if self.data_stats.get('categories_available') else 'Não'}")
            ...
            print(
                f"   • Total de arquivos CSV: {self.data_stats.get('csv_files', 0)}")
            print(
                f"   • Arquivos Nubank: {self.data_stats.get('nubank_files', 0)}")
            print(
                f"   • Pastas verificadas: {len(self.data_stats.get('folders_checked', []))}")
    ...
    def execute_config_v5(self):
        ...
        if choice == "1":
            ...
            if ai_choice in ['2', '3']:
                ...
                print("3. Obtenha API Key")
                api_key = input(
                    "Cole sua OPENAI_API_KEY (ou Enter para pular): ").strip()
                if api_key:
            ...
            print(
                "Modo avançado v5.0 - Planilhas organizadas automaticamente por ano/mês")
            print()
            ...
            if env_path.exists():
                ...
                groq_configured = "GROQ_API_KEY=" in env_content and len(
                    env_content.split("GROQ_API_KEY=")[1].split("\n")[0].strip()) > 0
                openai_configured = "OPENAI_API_KEY=" in env_content and len(
                    env_content.split("OPENAI_API_KEY=")[1].split("\n")[0].strip()) > 0
            ...
            print(
                f"   • Primeira execução: {'Sim' if self.config.get('first_run', True) else 'Não'}")
            print(
                f"   • Chatbot: {'✅' if self.system_status['chatbot_ready'] else '❌'}")
            print(
                f"   • Google Sheets Básico: {'✅' if self.system_status['google_sheets_basic'] else '❌'}")
            print(
                f"   • Google Sheets Avançado: {'✅' if self.system_status['google_sheets_advanced'] else '❌'}")
            print(f"   • IA: {'✅' if self.system_status['ai_ready'] else '❌'}")
            print(
                f"   • CSS Otimizado v3: {'✅' if self.system_status['css_optimized'] else '❌'}")
            print(
                f"   • Dependências: {'✅' if self.system_status['dependencies_ok'] else '❌'}")
...
def check_system_v5():
    ...
    sheets_basic = os.path.exists(
        "src/google_sheets_sync.py") and os.path.exists("credentials/google_credentials.json")
    print(f"   ☁️ Google Sheets Básico: {'✅' if sheets_basic else '❌'}")

    # Google Sheets avançado
    sheets_advanced = os.path.exists(
        "src/google_sheets_advanced.py") and os.path.exists("credentials/google_credentials.json")
    ...
    if os.path.exists(".env"):
        with open(".env", 'r', encoding='utf-8') as f:
            ...
            ai_ready = ("GROQ_API_KEY=" in env_content and len(env_content.split("GROQ_API_KEY=")[1].split("\n")[0].strip()) > 0) or \
                ("OPENAI_API_KEY=" in env_content and len(env_content.split(
                    "OPENAI_API_KEY=")[1].split("\n")[0].strip()) > 0)
