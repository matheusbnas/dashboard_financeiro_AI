...
class GoogleSheetsAdvanced:
    ...
    def connect(self):
        ...
        try:
            ...
            if not os.path.exists(self.credentials_path):
                ...
                print("\n📧 Emails que terão acesso automático:")
            ...
            print(f"📧 Compartilhamento automático configurado para: {', '.join(self.share_emails)}")
            return True
    ...
    def create_summary_worksheet(self, worksheet, df: pd.DataFrame, year: int, month: int):
        ...
        total_income = df[df['Valor'] > 0]['Valor'].sum() if not df[df['Valor'] > 0].empty else 0
        total_expenses = abs(df[df['Valor'] < 0]['Valor'].sum()) if not df[df['Valor'] < 0].empty else 0
        ...
        header_data = [
            ...
            ["💰 Total Receitas", f"R$ {total_income:,.2f}", f"Entradas de dinheiro"],
            ["💸 Total Despesas", f"R$ {total_expenses:,.2f}", f"Saídas de dinheiro"],
            ["💵 Saldo Líquido", f"R$ {balance:,.2f}", "Positivo" if balance > 0 else "Negativo"],
            ["🔢 Transações", f"{total_transactions:,}", "Número total de operações"],
            [""],
        ]
        
        # Adicionar análise por categoria se disponível
        if 'Categoria' in df.columns:
            expenses_by_category = df[df['Valor'] < 0].groupby('Categoria')['Valor'].sum().abs().sort_values(ascending=False)
            ...
            for i, (category, value) in enumerate(expenses_by_category.head(5).items(), 1):
                ...
                header_data.append([f"{i}º", category, f"R$ {value:,.2f}", f"{percentage:.1f}%"])
        ...
        if 'Descrição' in df.columns:
            ...
            for i, (establishment, freq) in enumerate(frequent_establishments.items(), 1):
                percentage = (freq / total_transactions * 100) if total_transactions > 0 else 0
                header_data.append([f"{i}º", establishment[:40], f"{freq}x", f"{percentage:.1f}%"])
    ...
    def create_category_worksheet(self, worksheet, df: pd.DataFrame, year: int, month: int):
        ...
        category_analysis.columns = ['Total', 'Quantidade', 'Média', 'Primeira_Data', 'Última_Data']
        category_analysis = category_analysis.sort_values('Total', key=abs, ascending=False)
        ...
        for category, row in category_analysis.iterrows():
            percentage = (abs(row['Total']) / total_abs * 100) if total_abs > 0 else 0
            data.append([
                category,
                f"R$ {row['Total']:,.2f}",
                int(row['Quantidade']),
    ...
    def create_establishments_worksheet(self, worksheet, df: pd.DataFrame, year: int, month: int):
        ...
        establishment_analysis.columns = ['Total_Gasto', 'Frequência', 'Gasto_Médio', 'Primeira_Compra', 'Última_Compra']
        establishment_analysis = establishment_analysis.sort_values('Frequência', ascending=False)
        ...
        for establishment, row in establishment_analysis.iterrows():
            frequency_percentage = (row['Frequência'] / total_transactions * 100) if total_transactions > 0 else 0
            data.append([
                establishment,
    ...
    def create_fixed_var_worksheet(self, worksheet, df: pd.DataFrame, year: int, month: int):
        ...
        for cost_type, row in fixed_var_analysis.iterrows():
            percentage = (abs(row['Total']) / total_expenses * 100) if total_expenses > 0 else 0
            data.append([
                cost_type,
    ...
    def process_all_data(self, df: pd.DataFrame):
        ...
        for year in years:
            ...
            for _, period in year_periods.iterrows():
                ...
                month_data = df[(df['Ano'] == year) & (df['Mês'] == month)].copy()
                
                if not month_data.empty:
                    print(f"   📅 Processando {datetime(year, month, 1).strftime('%B')} {year}...")
        ...
        print(f"📧 Todas as planilhas compartilhadas com: {', '.join(self.share_emails)}")
...
def load_financial_data():
    ...
    if is_nubank_data:
        print(f"💳 {len(nubank_files)} arquivo(s) Nubank detectado(s) - processamento otimizado")
...
def main():
    ...
    print(f"   • Período completo: {df['Data'].min().date()} até {df['Data'].max().date()}")
